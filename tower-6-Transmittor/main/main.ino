...
#define CLIENT_ID   "TOWER_6_LEVEL" //client ID | Unique ID for SUMP & motor

#define DOM_LEVELTOPIC "/towersix/dom/level"
#define FLU_LEVELTOPIC "/towersix/flu/level"

#define ONLINESTATUS "/towersix/online"

#define DOM_MOTORSTATUS "/towersix/dom/motorstatus"
#define FLU_MOTORSTATUS "/towersix/flu/motorstatus"

#define DOM_DASH_MOTORSTATUS "/towersix/dom/dashStatus"
...
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xCF, 0xFE, 0xED };
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.println("-----------------------");
  if (strcmp(messageBuffer, "true") == 0 && strcmp(topic, DOM_DASHBOARD_SWITCH) == 0) { //for T6 DOM
    Serial.println("T6-DOM: Dashboard switch On ");
    mqttClient.publish(MESSAGES, "T6-DOM: Dashboard switch On ");
  ...
  if (strcmp(messageBuffer, "false") == 0 && strcmp(topic, DOM_DASHBOARD_SWITCH) == 0) { //for T6 DOM
    Serial.println("T6-DOM: Dashboard switch Off");
    mqttClient.publish(MESSAGES, "T6-DOM: Dashboard switch Off");
  ...
  //--flu
  if (strcmp(messageBuffer, "true") == 0 && strcmp(topic, FLU_DASHBOARD_SWITCH) == 0) { //for T6 DOM
    Serial.println("T6-FLU: Dashboard switch On ");
    mqttClient.publish(MESSAGES, "T6-FLU: Dashboard switch On ");
  ...
  if (strcmp(messageBuffer, "false") == 0 && strcmp(topic, FLU_DASHBOARD_SWITCH) == 0) { //for T6 DOM
    Serial.println("T6-FLU: Dashboard switch Off");
    mqttClient.publish(MESSAGES, "T6-FLU: Dashboard switch Off");
    Serial.println("Motor OFF");//full
...
void setup() {
  ...
  Serial.println(F("Tower 6 Level Testor"));
...
void dom_dryRunCheck() {
  ...
  if (dom_motorOnFlag) {
    if (millis() - dom_onTime >= dom_interval) {
      ...
      mqttClient.publish(DRYRUNSTATUS, "Dry Run: Tower 6 Domestic motor ");
  ...
  if (dom_degenTimer) {
    ...
    if (millis() - dom_degenStartTime >= dom_degenInterval) {
      mqttClient.publish(DEGENSTATUS, "Degen Timer: Tower 6 Domestic motor ");
      ...
      if (dom_degenCount < DEGENCOUNT) {
        ...
        mqttClient.publish(MESSAGES, "T6 - DOM DRY RUN");
...
void flu_dryRunCheck() {
  ...
  if (flu_motorOnFlag) {
    if (millis() - flu_onTime >= dom_interval) {
      ...
      mqttClient.publish(DRYRUNSTATUS, "Dry Run: Tower 6 Flush motor ");
  ...
  if (flu_degenTimer) {
    ...
    if (millis() - flu_degenStartTime >= dom_degenInterval) {
      Serial.println(F("Staring again degen count ------"));
      mqttClient.publish(DEGENSTATUS, "Degen Timer: Tower 6 Flush motor ");
      if (flu_degenCount < DEGENCOUNT) {
        ...
        mqttClient.publish(MESSAGES, "T6 Flush tank DR");
        //Degen Count completed
        //Inform to check the overall system
      }
    }
...
bool dom_sendData() {
  ...
  if (dom_motorConditionFlag && !dom_degenTimer && !dom_dryRunError) {
    ...
    Serial.println(F("Turn ON Tower 6 DOM MOTOR"));
    ...
    Serial.println(F("Turn OFF Tower 6 DOM MOTOR"));
    sendStatusFlag = mqttClient.publish(DOM_MOTORSTATUS, msgBuffer);
    if (sendStatusFlag == false) {
      Serial.println("Error while publishsing ");
      return false;
...
bool flu_sendData() {
  ...
  if (flu_motorConditionFlag && !flu_degenTimer && !flu_dryRunError) {
    msgBuffer = "turnOnMotorOne";
    Serial.println(F("Turn ON Tower 6 FLU MOTOR"));
    ...
    msgBuffer = "turnOffMotorOne";
    Serial.println(F("Turn OFF Tower 6 FLU MOTOR"));
    sendStatusFlag = mqttClient.publish(FLU_MOTORSTATUS, msgBuffer);
    if (sendStatusFlag == false) {
      Serial.println("Error while publishsing ");
      return false;

...
//#define FLU_DEGENSTATUS "/towertwo/flu/degen"

#define MESSAGES "/towertwo/dom/messages"
//#define FLU_MESSAGES "/towertwo/flu/messages"

#define DOM_DASHBOARD_SWITCH "/towertwo/dom/switch"
#define FLU_DASHBOARD_SWITCH "/towertwo/flu/switch"
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.println("-----------------------");
  if (strcmp(messageBuffer, "true") == 0 && strcmp(topic, DOM_DASHBOARD_SWITCH) == 0) { //for T2 DOM
    Serial.println("T2-DOM: Dashboard switch On ");
    mqttClient.publish(MESSAGES, "T2-DOM: Dashboard switch On ");
  ...
  if (strcmp(messageBuffer, "false") == 0 && strcmp(topic, DOM_DASHBOARD_SWITCH) == 0) { //for T2 DOM
    Serial.println("T2-DOM: Dashboard switch Off");
    mqttClient.publish(MESSAGES, "T2-DOM: Dashboard switch Off");
  ...
  //--flu
  if (strcmp(messageBuffer, "true") == 0 && strcmp(topic, FLU_DASHBOARD_SWITCH) == 0) { //for T2 DOM
    Serial.println("T2-FLU: Dashboard switch On ");
    mqttClient.publish(MESSAGES, "T2-FLU: Dashboard switch On ");
  ...
  if (strcmp(messageBuffer, "false") == 0 && strcmp(topic, FLU_DASHBOARD_SWITCH) == 0) { //for T2 DOM
    Serial.println("T2-FLU: Dashboard switch Off");
    mqttClient.publish(MESSAGES, "T2-FLU: Dashboard switch Off");
    Serial.println("Motor OFF");//full
...
void setup() {
  ...
  Serial.println(F("Tower 2 Level Testor"));
...
void dom_dryRunCheck() {
  ...
  if (dom_motorOnFlag) {
    if (millis() - dom_onTime >= dom_interval) {
      ...
      mqttClient.publish(DRYRUNSTATUS, "Dry Run: Tower 2 Domestic motor ");
  ...
  if (dom_degenTimer) {
    ...
    if (millis() - dom_degenStartTime >= dom_degenInterval) {
      mqttClient.publish(DEGENSTATUS, "Degen Timer: Tower 2 Domestic motor ");
      ...
      if (dom_degenCount < DEGENCOUNT) {
        ...
        mqttClient.publish(MESSAGES, "T2 - DOM DRY RUN");
...
void flu_dryRunCheck() {
  ...
  if (flu_motorOnFlag) {
    if (millis() - flu_onTime >= dom_interval) {
      ...
      mqttClient.publish(DRYRUNSTATUS, "Dry Run: Tower 2 Flush motor ");
  ...
  if (flu_degenTimer) {
    ...
    if (millis() - flu_degenStartTime >= dom_degenInterval) {
      Serial.println(F("Staring again degen count ------"));
      mqttClient.publish(DEGENSTATUS, "Degen Timer: Tower 2 Flush motor ");
      if (flu_degenCount < DEGENCOUNT) {
        ...
        mqttClient.publish(MESSAGES, "T2 Flush tank DR");
        //Degen Count completed
        //Inform to check the overall system
      }
    }
...
bool dom_sendData() {
  ...
  if (dom_motorConditionFlag && !dom_degenTimer && !dom_dryRunError) {
    ...
    Serial.println(F("Turn ON Tower 2 DOM MOTOR"));
    ...
    Serial.println(F("Turn OFF Tower 2 DOM MOTOR"));
    sendStatusFlag = mqttClient.publish(DOM_MOTORSTATUS, msgBuffer);
    if (sendStatusFlag == false) {
      Serial.println("Error while publishsing ");
      return false;
...
bool flu_sendData() {
  ...
  if (flu_motorConditionFlag && !flu_degenTimer && !flu_dryRunError) {
    msgBuffer = "turnOnMotorOne";
    Serial.println(F("Turn ON Tower 2 FLU MOTOR"));
    ...
    msgBuffer = "turnOffMotorOne";
    Serial.println(F("Turn OFF Tower 2 FLU MOTOR"));
    sendStatusFlag = mqttClient.publish(FLU_MOTORSTATUS, msgBuffer);
    if (sendStatusFlag == false) {
      Serial.println("Error while publishsing ");
      return false;
